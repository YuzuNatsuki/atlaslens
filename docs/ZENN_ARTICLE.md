# AtlasLens を作った：EM の「見落とし」を減らす Agentic AI Co-pilot
...
エンジニアリングマネージャー（EM）の仕事は、日報、1on1、OKR、会議メモ、組織情報など、散らばったテキストからチームの状態を読み取ることです。

ただ、現実には全部を毎日読むのは難しい。日報の一文にある「ちょっとした違和感」や、1on1 が空いているメンバーのサインを見落とすことがあります。

そこで **AtlasLens** を作りました。

AtlasLens は、EM のための **Agentic AI Co-pilot** です。既存のテキスト資産を Azure 上の AI エージェントが横断的に読み、EM が「今日見るべきこと」「今週声をかけるべき人」「体制変更の影響」を判断しやすくします。
...
AtlasLens が目指したのは「EM を AI に置き換える」ことではありません。EM がチームに向き合うための注意力を補うことです。
...
EM が自然文で質問すると、GPT-4o が必要なツールを選び、最大 4 ラウンドまで自律的に情報を取得します。
...
UI は途中で大きく作り直し、現在は左サイドバー + トップバー構成です。EM / Admin / Member でナビゲーションが変わります。
...
| Line-of-sight RBAC | EM が見られる範囲を組織ツリーで厳密に制限する |
...
AtlasLens は、EM のための Agentic AI Co-pilot として、以下を 1 つの体験にまとめました。
...
EM の仕事は、人を見る仕事です。AI が決めるのではなく、AI が散らばった情報を整理し、EM が対話に時間を使えるようにする。
