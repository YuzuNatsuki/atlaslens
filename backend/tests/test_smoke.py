...
def test_insight_action_pdca_lifecycle(monkeypatch):
    """Create → in_progress → resolved should preserve the full history."""
    from app.services import insight_actions

    _fake_artefact_store(monkeypatch, insight_actions)

    action = insight_actions.create_action(
        created_by="em001",
        title="渡辺さんの離職リスクを確認",
        source_kind="team-summary-range",
        source_key="2026-05-13_2026-05-19",
        signal_kind="retention",
        member_id="mem004",
        evidence_dates=["2026-05-14", "2026-05-18"],
        details="期間サマリーで外部選考の記述あり",
        initial_note="期間サマリーから追跡開始",
    )
    assert action["status"] == "open"
    assert len(action["history"]) == 1

    progressed = insight_actions.update_action(
        action["id"],
        actor="em001",
        status="in_progress",
        note="1on1 を今週中に設定済み",
    ...
    assert len(progressed["history"]) == 2

    resolved = insight_actions.update_action(
        action["id"],
        actor="em001",
        status="resolved",
        note="1on1 後、本人の意向確認済み",
    )
...
def test_insight_action_invalid_status():
    """update_action must reject statuses outside the allowed set."""
    from app.services import insight_actions

    try:
        insight_actions.update_action(
            "nonexistent", actor="em001", status="bogus-status"
        )
